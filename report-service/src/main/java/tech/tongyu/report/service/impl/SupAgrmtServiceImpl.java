...
import tech.tongyu.transform.dao.repo.history.SupHistoryRepo;
import tech.tongyu.transform.dao.source.SupAgrmtDbo;
import tech.tongyu.transform.dao.repo.source.SupAgrmtRepo;
 ...
 */
@Service
public class SupAgrmtServiceImpl implements SupAgrmtService {

    @Value("${spring.upload.location}")
    private String filePath;

    private final BaseElementRepo baseElementRepo;
    private final SupAgrmtRepo supAgrmtRepo;
    ...
    private final SupHistoryRepo supHistoryRepo;
    private final AttachmentConfigService attachmentConfigService;

    @Autowired
    public SupAgrmtServiceImpl(BaseElementRepo baseElementRepo,
                               SupAgrmtRepo supAgrmtRepo,
        ...
        this.supHistoryRepo = supHistoryRepo;
        this.attachmentConfigService = attachmentConfigService;
    }

    @Override
    public List<SupAgrmtDTO> listSupAgrmt() {
        List<SupAgrmtDbo> products = supAgrmtRepo.getReport();
        return products.stream()
                .map(product -> SupAgrmtDTO.fromJson(product.getSup()))
                .collect(Collectors.toList());
    }

    @Override
    public String getServiceType() {
        return InterfaceInfoEnum.SUPPLEMENTARY.code();
    }
    ...
    public Boolean executeTransTask() {
        ...
        List<SupAgrmtDbo> reports = supAgrmtRepo.getReport();
        List<Map<String, Object>> collect = reports.stream()
                .map(report -> (Map<String, Object>) JsonUtils.mapper.convertValue(report.getSup(), Map.class))
                .collect(Collectors.toList());
        List<Map<String, Object>> standardValues = checkAndSupplementService.getStandardValue(collect, elements);
        for (int i = 0; i < reports.size(); i++) {
            SupAgrmtDbo report = reports.get(i);
            Map<String, Object> map = standardValues.get(i);
            report.setSup(JsonUtils.mapper.valueToTree(map));
        }
        supAgrmtRepo.saveAll(reports);
        return true;
    }
    ...
    public String executeGenerateTask(OperationEnum operation) {
        ...
        LinkedHashMap<String, String> requestHeader = StringForReportUtils.buildRequestHeader(config.getSenderCode()
                ...
                , now
                , version
                , InterfaceInfoEnum.SUPPLEMENTARY.code()
                , operation);
        //获取草稿箱数据
        List<SupAgrmtDbo> reports = supAgrmtRepo.getReport();
        ...
        for (int i = 0; i < reports.size(); i++) {
            SupAgrmtDbo report = reports.get(i);
            //json -> dto
            SupAgrmtDTO reportData = SupAgrmtDTO.fromJson(report.getSup(), Optional.of(operation));
            //生成execId
        ...
        String xmlName = StringForReportUtils.generateXmlName(config.getSenderCode(), config.getReceiverCode(), now,
                version, InterfaceInfoEnum.SUPPLEMENTARY.code(), operation);
        ...
        Dom4jXmlManager.createXml(xmlPath, requestHeader, data, InterfaceInfoEnum.SUPPLEMENTARY);
        //复制附件到指定目录
        attachmentConfigService.copyAttachmentToCompress(InterfaceInfoEnum.SUPPLEMENTARY.code(), currentDir);
        //打包zip
    ...
    public List<Map<String, Object>> listReportMap() {
        List<SupAgrmtDbo> products = supAgrmtRepo.findAll();
        return products.stream()
    ...
    public Boolean saveReport(List<Map<String, Object>> data) {
        supAgrmtRepo.deleteReportList();
        List<SupAgrmtDbo> list = data.stream().map(agreement -> {
            JsonNode node = JsonUtils.mapper.valueToTree(agreement);
            String bizId = null;
            if (node.has(bizId_field)) {
                bizId = node.get(bizId_field).textValue();
            }
            return new SupAgrmtDbo(node, bizId, false);
        }).collect(Collectors.toList());
        supAgrmtRepo.saveAll(list);
        return true;
    }

    @Override
    public Boolean updateReport(String bizId, Map<String, Object> data) {
        Optional<SupAgrmtDbo> report = supAgrmtRepo.getReportByBizID(bizId);
        if (report.isPresent()) {
            SupAgrmtDbo product = report.get();
            product.setSup(JsonUtils.mapper.valueToTree(data));
            supAgrmtRepo.save(product);
            return true;
        } else {
    ...
    public Boolean deleteReport(String bizId) {
        supAgrmtRepo.deleteByBizId(bizId);
        return true;
    }
    ...
    public ResponseData archiveData(ResponseCommon response) {
        ...
        responseData.setInterfaceInfo("补充协议");
        List<ResponseBodyCommon> bodyCommons = response.getBody();
        List<SupAgrmtDbo> reportList = new ArrayList<>();
        List<ResponseDetail> details = new ArrayList<>();
        for (ResponseBodyCommon bodyCommon : bodyCommons) {
            ResponseDetail responseDetail = new ResponseDetail();
            Optional<SupAgrmtDbo> reportOptional = supAgrmtRepo.getReportByExecID(bodyCommon.getExecID());
            if (reportOptional.isPresent()) {
                SupAgrmtDbo report = reportOptional.get();
                SupAgrmtDTO reportData = SupAgrmtDTO.fromJson(report.getSup());
                ...
                //数据归档
                SupHistoryDbo history;
                Optional<SupHistoryDbo> historyOptional = supHistoryRepo.getReportByBizID(bodyCommon.getBizID());
                if (historyOptional.isPresent()) {
                    history = historyOptional.get();
                    history.setSup(data);
                } else {
                    history = new SupHistoryDbo(data, bodyCommon.getBizID());
                }
                supHistoryRepo.save(history);
            } else {
        ...
        supAgrmtRepo.saveAll(reportList);
        return responseData;
    }
}
