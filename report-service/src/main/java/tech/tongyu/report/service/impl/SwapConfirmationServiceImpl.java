...
import tech.tongyu.transform.dao.repo.source.SwapConfirmationRepo;
...
@Service
public class SwapConfirmationServiceImpl implements SwapConfirmationService {

    @Value("${spring.upload.location}")
    private String filePath;

    private final BaseElementRepo baseElementRepo;
    private final SwapConfirmationRepo swapConfirmationRepo;
    ...
    private final SwapConfirmationHistoryRepo swapConfirmationHistoryRepo;
    private final AttachmentConfigService attachmentConfigService;

    @Autowired
    public SwapConfirmationServiceImpl(BaseElementRepo baseElementRepo,
                                       SwapConfirmationRepo swapConfirmationRepo,
                                       ...
                                       SwapConfirmationHistoryRepo swapConfirmationHistoryRepo,
                                       AttachmentConfigService attachmentConfigService) {
        this.baseElementRepo = baseElementRepo;
        this.swapConfirmationRepo = swapConfirmationRepo;
        ...
        this.swapConfirmationHistoryRepo = swapConfirmationHistoryRepo;
        this.attachmentConfigService = attachmentConfigService;
    }
    
    @Override
    public List<SwapConfirmationDTO> listSwapConfirmation() {
        List<SwapConfirmationDbo> products = swapConfirmationRepo.getReport();
        return products.stream()
                .map(product -> SwapConfirmationDTO.fromJson(product.getSwapconfirm()))
                .collect(Collectors.toList());
    }

    @Override
    public String getServiceType() {
        return InterfaceInfoEnum.CONFIRMATION_OF_SWAP.code();
    }
    ...
    public Boolean executeTransTask() {
        ...
        List<SwapConfirmationDbo> reports = swapConfirmationRepo.getReport();
        List<Map<String, Object>> collect = reports.stream()
                .map(report -> (Map<String, Object>) JsonUtils.mapper.convertValue(report.getSwapconfirm(), Map.class))
                .collect(Collectors.toList());
        List<Map<String, Object>> standardValues = checkAndSupplementService.getStandardValue(collect, elements);
        for (int i = 0; i < reports.size(); i++) {
            SwapConfirmationDbo report = reports.get(i);
            Map<String, Object> map = standardValues.get(i);
            report.setSwapconfirm(JsonUtils.mapper.valueToTree(map));
        }
        swapConfirmationRepo.saveAll(reports);
        return true;
    }
    ...
    public String executeGenerateTask(OperationEnum operation) {
        ...
        LinkedHashMap<String, String> requestHeader = StringForReportUtils.buildRequestHeader(config.getSenderCode()
                ...
                , now
                , version
                , InterfaceInfoEnum.CONFIRMATION_OF_SWAP.code()
                , operation);
        //获取草稿箱数据
        List<SwapConfirmationDbo> reports = swapConfirmationRepo.getReport();
        ...
        for (int i = 0; i < reports.size(); i++) {
            SwapConfirmationDbo report = reports.get(i);
            SwapConfirmationDTO reportData = SwapConfirmationDTO.fromJson(report.getSwapconfirm(), Optional.of(operation));
        ...
        String xmlName = StringForReportUtils.generateXmlName(config.getSenderCode(), config.getReceiverCode(), now,
                version, InterfaceInfoEnum.CONFIRMATION_OF_SWAP.code(), operation);
        ...
        Dom4jXmlManager.createXml(xmlPath, requestHeader, data, InterfaceInfoEnum.CONFIRMATION_OF_SWAP);
        //复制附件到指定目录
        attachmentConfigService.copyAttachmentToCompress(InterfaceInfoEnum.CONFIRMATION_OF_SWAP.code(), currentDir);
        //打包zip
    ...
    public List<Map<String, Object>> listReportMap() {
        List<SwapConfirmationDbo> products = swapConfirmationRepo.findAll();
        return products.stream()
    ...
    public Boolean saveReport(List<Map<String, Object>> data) {
        swapConfirmationRepo.deleteReportList();
        List<SwapConfirmationDbo> list = data.stream().map(agreement -> {
            JsonNode node = JsonUtils.mapper.valueToTree(agreement);
            String bizId = null;
            if (node.has(bizId_field)) {
                bizId = node.get(bizId_field).textValue();
            }
            return new SwapConfirmationDbo(node, bizId, false);
        }).collect(Collectors.toList());
        swapConfirmationRepo.saveAll(list);
        return true;
    }

    @Override
    public Boolean updateReport(String bizId, Map<String, Object> data) {
        Optional<SwapConfirmationDbo> report = swapConfirmationRepo.getReportByBizID(bizId);
        if (report.isPresent()) {
            SwapConfirmationDbo product = report.get();
            product.setSwapconfirm(JsonUtils.mapper.valueToTree(data));
            swapConfirmationRepo.save(product);
            return true;
        } else {
    ...
    public Boolean deleteReport(String bizId) {
        swapConfirmationRepo.deleteByBizId(bizId);
        return true;
    }
    ...
    public ResponseData archiveData(ResponseCommon response) {
        ...
        responseData.setInterfaceInfo("互换交易确认书");
        List<ResponseBodyCommon> bodyCommons = response.getBody();
        List<SwapConfirmationDbo> reportList = new ArrayList<>();
        List<ResponseDetail> details = new ArrayList<>();
        for (ResponseBodyCommon bodyCommon : bodyCommons) {
            ResponseDetail responseDetail = new ResponseDetail();
            Optional<SwapConfirmationDbo> reportOptional = swapConfirmationRepo.getReportByExecID(bodyCommon.getExecID());
            if (reportOptional.isPresent()) {
                SwapConfirmationDbo report = reportOptional.get();
                SwapConfirmationDTO reportData = SwapConfirmationDTO.fromJson(report.getSwapconfirm());
                ...
                //数据归档
                SwapConfirmationHistoryDbo history;
                Optional<SwapConfirmationHistoryDbo> historyOptional = swapConfirmationHistoryRepo.getReportByBizID(bodyCommon.getBizID());
                if (historyOptional.isPresent()) {
                    history = historyOptional.get();
                    history.setSwapconfirm(data);
                } else {
                    history = new SwapConfirmationHistoryDbo(data, bodyCommon.getBizID());
                }
                swapConfirmationHistoryRepo.save(history);
            } else {
        ...
        swapConfirmationRepo.saveAll(reportList);
        return responseData;
    }
}
