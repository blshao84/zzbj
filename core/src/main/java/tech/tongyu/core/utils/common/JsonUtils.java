...
public class JsonUtils {
    ...
    public static void removeTagIfAbsent(JsonNode jsonNode) {
        if (jsonNode.has(BCT_JACKSON_TYPE_TAG)) {
            ((ObjectNode) jsonNode).remove(BCT_JACKSON_TYPE_TAG);
        }
    }
    ...
    private static JavaType getLinkedHashMapType(Class<?>... elementClasses) {
        return mapper.getTypeFactory().constructParametricType(LinkedHashMap.class, elementClasses);
    }
    ...
    public static LinkedHashMap<String, Object> fromJsonToLinkedHashMap(String context) throws SerializationException {
        ...
        if (StringUtils.isNotBlank(context)) {
            JavaType type = getLinkedHashMapType(String.class, Object.class);
            try {
                resultMap = mapper.readValue(context, type);
            } catch (IOException e) {
                throw new SerializationException(String.format("Failed to decode: [%s]", context));
    ...
    public static <T> T copyProperties(Class<T> clazz, Object source) throws SerializationException {
        String context=objectToJsonString(source);
        try{
            return mapper.readValue(context, clazz);
        }catch (IOException e){
            throw new SerializationException(String.format("Failed to decode: [%s]", context));
