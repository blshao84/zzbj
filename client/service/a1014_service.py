    ...
    if additional:
        for a in additional:
            ...
            if additional_list:
                ...
                additional_map[id] = [a]
    for m in main:
        additional_list = additional_map.get(m.get('DerivativesQuarterlyReportAnnexTuple'))
        if additional_list:
            m['DerivativesQuarterlyReportAnnexTuple'] = additional_list
        else:
            m['DerivativesQuarterlyReportAnnexTuple'] = []
...
def save_data(data):
    url = 'http://{ip}:{port}/pr-quarter/excel/prepare'.format(ip=utils.ip(), port=utils.port())
    params = {
        "params": data
    }
...
if __name__ == '__main__':
    ...
    report.xlsxFileAbsName = 'D:/test/A1014.xlsx'
