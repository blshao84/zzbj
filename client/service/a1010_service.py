    ...
    if additional:
        for a in additional:
            ...
            if additional_list:
                ...
                additional_map[id] = [a]
    for m in main:
        additional_list = additional_map.get(m.get('OtherReportFileTuple'))
        if additional_list:
            m['OtherReportFileTuple'] = additional_list
        else:
            m['OtherReportFileTuple'] = []
...
def save_data(data):
    url = 'http://{ip}:{port}/other-report/excel/prepare'.format(ip=utils.ip(), port=utils.port())
    params = {
        "params": data
    }
...
if __name__ == '__main__':
    ...
    report.xlsxFileAbsName = 'D:/test/A1010.xlsx'
